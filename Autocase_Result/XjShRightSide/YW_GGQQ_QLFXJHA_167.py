...
class YW_GGQQ_QLFXJHA_167(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_QLFXJHA_167')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_QLFXJHA_167(self):
        title = '卖平（权利方平仓）：FOK限价全成或撤销-验券（持仓刚好）（下单数量=可卖持仓）'
        # 定义当前测试用例的期待值
