...
class YW_GGQQ_QLFXJHA_127(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_QLFXJHA_127')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_QLFXJHA_127(self):
        title = '买开（权利方开仓）：FOK限价全成或撤销-验资（可用资金刚好）（可用资金=下单金额+费用）'
        # 定义当前测试用例的期待值
