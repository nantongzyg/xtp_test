...
class YW_GGQQ_QLFXJHA_162(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_QLFXJHA_162')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_QLFXJHA_162(self):
        title = '卖平（权利方平仓）：FOK限价全成或撤销-验资（可用资金刚好）（下单金额<费用&&可用资金=（费用-下单金额））'
        # 定义当前测试用例的期待值
