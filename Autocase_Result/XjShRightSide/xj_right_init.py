...
class xj_right_init(xtp_test_case):
    # YW_GGQQ_YWFXJHA_024

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_QLFXJHA_011')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_xj_right_init(self):
        title = '卖开（义务方开仓）：T日增加10001032和20002438多头持仓'
        # 定义当前测试用例的期待值
        ...
        for i in xrange(3):
            for code, quantity in {'10001032': 10, '20002438': 10000}.items():
                stkparm = QueryStkPriceQty(code, '1', '*', '1', '0', '*', case_goal['期望状态'], Api)
                # 如果下单参数获取失败，则用例失败
