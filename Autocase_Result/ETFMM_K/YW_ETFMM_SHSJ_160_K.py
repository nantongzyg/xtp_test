...
class YW_ETFMM_SHSJ_160_K(xtp_test_case):
    # YW_ETFMM_SHSJ_160
    def test_YW_ETFMM_SHSJ_160_K(self):
        title = '交易日五档即成转撤销卖-T+0买→T+0卖（未持有证券代码）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('515088', '1', '15', '2', '0', 'B', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        ...
        # T+0买
        title = '上海Ａ股交易日五档即成转撤销卖（未持有证券代码）-T+0买'
        ...
        self.assertEqual(rs['用例测试结果'], True)
        # T+0卖
        title = '上海Ａ股交易日五档即成转撤销卖（未持有证券代码）-T+0卖'
        logger.warning(title)
