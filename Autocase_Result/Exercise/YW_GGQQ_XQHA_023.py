...
class YW_GGQQ_XQHA_023(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_XQHA_023')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_XQHA_023(self):
        title = '认购行权-验资--行权日15:00前：可用资金刚好（可用资金=行权价格*合约单位*数量+费用）'
        # 定义当前测试用例的期待值
