...
class YW_GGQQ_XQHA_042(xtp_test_case):
    # YW_GGQQ_XQHA_042

    def test_YW_GGQQ_XQHA_042(self):
        title='认购行权-非行权日进行行权'
        #定义当前测试用例的期待值
        ...
        case_goal = {
            '期望状态': '废单',
            'errorID': 11010330,
            'errorMSG': 'Failed to check exercise date in options executive.',
            #'errorMSG': queryOrderErrorMsg(11010330),
            '是否生成报单': '是',
            '是否是撤废': '否',
            'xtp_ID': 0,
            'cancel_xtpID': 0,
        ...
        stkparm = QueryStkPriceQty('11002637', '1', '*', '1', '0', 'C', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
