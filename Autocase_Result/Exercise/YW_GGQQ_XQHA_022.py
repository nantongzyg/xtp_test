...
class YW_GGQQ_XQHA_022(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_XQHA_022')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_XQHA_022(self):
        title = '认购行权-验资--行权日15:00前：可用资金不足（可用资金=行权价格*合约单位*数量+费用-0.01）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('11002388', '1', '*', '1', '0', 'C', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] is False:
            ...
            wt_reqs = {
                'business_type':Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_EXECUTE'],
                'order_client_id':1,
