...
class YW_GGQQ_XQHA_070(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_XQHA_070')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_XQHA_070(self):
        title = '认沽行权-验券--行权日15:00前：合约持仓不足（合约可用数量<最大可行权合约数量）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('10001033', '1', '*', '1', '0', 'P', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
