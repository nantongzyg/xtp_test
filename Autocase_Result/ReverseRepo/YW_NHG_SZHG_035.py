...
class YW_NHG_SZHG_035(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_NHG_SZHG_035')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_NHG_SZHG_035(self):
        title = '可用资金充足-深A限价买入（可用资金=下单金额+费用+0.01）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('999999', '2', '12', '2', '0', 'S', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
