...
class YW_GGQQ_QLFSJHA_069(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_QLFSJHA_069')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_QLFSJHA_069(self):
        title = '卖平（权利方平仓）：市价剩余转限价-验券（持仓刚好）（下单数量=可卖持仓）'
        # 定义当前测试用例的期待值
