...
class YW_ETFMM_SHSJ_115_D(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFMM_SHSJ_115_D')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFMM_SHSJ_115_D(self):
        title = '可用资金正好-沪A五档即成转撤销买（可用资金=下单金额+费用）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('514000', '1', '14', '2', '0', 'B', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
