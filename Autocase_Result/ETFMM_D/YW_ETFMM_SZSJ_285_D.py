...
class YW_ETFMM_SZSJ_285_D(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFMM_SZSJ_285_D')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFMM_SZSJ_285_D(self):
        title = '可用资金正好-深A本方最优卖（可用资金+卖出金额=费用）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('157146', '2', '14', '2', '0', 'S', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
