...
class FK_FKYW_FJJJSS_244(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.delete_cur_risk()
        sql_transfer.insert_cur_risk('FK_FKYW_GPWT_631')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        time.sleep(2)
        Api.trade.Login()

    def test_FK_FKYW_FJJJSS_244(self):
        title = '默认rule43,rule0=1'
        #定义当前测试用例的期待值
        ...
        stkparm = QueryStructuredFundInfo('999999', '2', '26', '2', '0', '0',
                                          case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] is False:
            ...
            count = 1
            max_count = 5000
            filename1 = 'FK_FKYW_FJJJSS_244_order1'
            filename2 = 'FK_FKYW_FJJJSS_244_order2'
            ...
            # 校验订单是否正确触发风控
            rule43_check(filename1, filename2)
