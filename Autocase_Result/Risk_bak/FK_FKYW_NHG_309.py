...
class FK_FKYW_NHG_309(xtp_test_case):
    ...
    def test_FK_FKYW_NHG_309(self):
        ...
        stkparm = QueryStkPriceQty('999999', '1', '12', '2', '0', 'S',
                                   case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] == False:
            ...
            count = 1
            max_count = 101
            filename = 'FK_FKYW_NHG_309_order'
            insert_orders_sleep(count, max_count, Api, case_goal, wt_reqs, filename)
            time.sleep(3)

        stkparm2 = QueryStkPriceQty('999999', '1', '12', '2', '0', 'S',
                                    case_goal['期望状态'], Api)
        if stkparm2['返回结果'] == False:
            rs = {
                '用例测试结果':stkparm2['返回结果'],
                '测试错误原因':'获取下单参数失败,'+stkparm2['错误原因'],
            }
            self.assertEqual(rs['用例测试结果'], True)
            ...
            wt_reqs = {
                ...
                'order_client_id': 1,
                'market': Api.const.XTP_MARKET_TYPE['XTP_MKT_SH_A'],
                'ticker': stkparm2['证券代码'],
                'side': Api.const.XTP_SIDE_TYPE['XTP_SIDE_SELL'],
                'price_type': Api.const.XTP_PRICE_TYPE['XTP_PRICE_LIMIT'],
                'price': stkparm2['随机中间价'],
                'quantity': 1000
            }
            count = 1
            max_count = 1
