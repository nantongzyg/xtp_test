...
class FK_FKYW_RZRQ_721(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.delete_cur_risk()
        sql_transfer.insert_cur_risk('FK_FKYW_RZRQ_721')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        time.sleep(2)
        Api.trade.Login()

    def test_FK_FKYW_RZRQ_721(self):
        ...
        if stkparm['返回结果'] == False:
            ...
            count = 1
            max_count = 10000
            filename1 = 'FK_FKYW_RZRQ_721_order1'
            filename2 = 'FK_FKYW_RZRQ_721_order2'
