...
def query_capital_stock(Api,orders,config,ticker,number):
    """
    查询资金和总持仓
    :param Api:
    ...
    fund_info = {}
    fund_info['fund'] = str(fund)
    orders[username] = fund_info
    ...
    if number == 1: 
        logger.info('''异常恢复前用户订单信息:%s'''%str(orders))
    elif number == 2:
        logger.info('''异常恢复前用户订单信息:%s'''%str(orders))
    ...
    if False:
        ...
        for user in order_info:
            for info in order_info[user]:
                logger.info("订单信息：%s"%str(order_info))
                logger.info("restart订单信息：%s"%str(restart_info))
...
def insert_order(Api,wt_reqs, user):
    """
    批量下单,支持股票买卖/ETF买卖/分级子基金买卖，不支持配股缴款，新股申购
    :param Api:
    :param wt_reqs:
    :param user:
    :return:
