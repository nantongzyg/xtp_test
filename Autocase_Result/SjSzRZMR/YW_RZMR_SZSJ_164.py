...
class YW_RZMR_SZSJ_164(xtp_test_case):
    # YW_RZMR_SZSJ_164 YW_RZMR_SZSJ_164
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_credit_asset('YW_RZMR_SZSJ_164')
        sql_transfer.transfer_credit_cash_fund_asset('YW_RZMR_SZSJ_164')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_RZMR_SZSJ_164(self):
        title = '五档即成转撤销买—单金额+手续费>融资头寸+可用资金'
        # 定义当前测试用例的期待值
