...
class YW_RZMR_SZSJ_005(xtp_test_case):
    # YW_RZMR_SZSJ_005
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.keep_stk_asset()
        sql_transfer.transfer_credit_asset('YW_RZMR_SZSJ_005')
        sql_transfer.transfer_credit_cash_fund_asset("YW_RZMR_SZSJ_005")
        sql_transfer.transfer_credit_debts('YW_RZMR_SZSJ_005')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_RZMR_SZSJ_005(self):
        title = '即成剩撤买--已成（费用>min）'
        # 定义当前测试用例的期待值
