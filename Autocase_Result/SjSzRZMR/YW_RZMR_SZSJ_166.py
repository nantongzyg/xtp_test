...
class YW_RZMR_SZSJ_166(xtp_test_case):
    # YW_RZMR_SZSJ_166 YW_RZMR_SZSJ_166
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_credit_asset('YW_RZMR_SZSJ_166')
        sql_transfer.transfer_credit_cash_fund_asset('YW_RZMR_SZSJ_166')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_RZMR_SZSJ_166(self):
        title = '全部成交或撤销买--融资头寸<下单金额+手续费<=融资头寸+可用资金'
        # 定义当前测试用例的期待值
