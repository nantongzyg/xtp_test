...
class YW_RZMR_SZSJ_172(xtp_test_case):
    # YW_RZMR_SZSJ_172 YW_RZMR_SZSJ_172
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_credit_asset('YW_RZMR_SZSJ_172')
        sql_transfer.transfer_credit_cash_fund_asset('YW_RZMR_SZSJ_172')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_RZMR_SZSJ_172(self):
        title = '对方最优转限价买--融资头寸<下单金额+手续费<=融资头寸+可用资金'
        # 定义当前测试用例的期待值
        ...
        case_goal = {
            '期望状态': '废单',
            'errorID': 11010502,
            'errorMSG': queryOrderErrorMsg(11010502),
            '是否生成报单': '是',
            '是否是撤废': '否',
            'xtp_ID': 0,
            'cancel_xtpID': 0,
        ...
        stkparm = QueryStkPriceQty('003271', '2', '0', '2', '0', 'B', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] is False:
            ...
            self.assertEqual(rs['用例测试结果'], True) # 18
