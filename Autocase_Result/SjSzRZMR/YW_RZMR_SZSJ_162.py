...
class YW_RZMR_SZSJ_162(xtp_test_case):
    # YW_RZMR_SZSJ_162 YW_RZMR_SZSJ_162
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_credit_asset('YW_RZMR_SZSJ_162')
        sql_transfer.transfer_credit_cash_fund_asset('YW_RZMR_SZSJ_162')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_RZMR_SZSJ_162(self):
        title = '五档即成转撤销买--下单金额+手续费<=融资头寸'
        # 定义当前测试用例的期待值
