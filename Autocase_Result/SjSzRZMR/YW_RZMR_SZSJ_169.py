...
class YW_RZMR_SZSJ_169(xtp_test_case):
    # YW_RZMR_SZSJ_169 YW_RZMR_SZSJ_169
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_credit_asset('YW_RZMR_SZSJ_169')
        sql_transfer.transfer_credit_cash_fund_asset('YW_RZMR_SZSJ_169')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_RZMR_SZSJ_169(self):
        title = '本方最优买--融资头寸<下单金额+手续费<=融资头寸+可用资金'
        # 定义当前测试用例的期待值
