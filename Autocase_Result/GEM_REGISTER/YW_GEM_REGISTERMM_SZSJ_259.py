...
class YW_GEM_REGISTERMM_SZSJ_259(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GEM_REGISTERMM_SZSJ_259')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GEM_REGISTERMM_SZSJ_259(self):
        title = '可用资金不足-深A本方最优买（可用资金=下单金额+费用-0.01）'
        # 定义当前测试用例的期待值
