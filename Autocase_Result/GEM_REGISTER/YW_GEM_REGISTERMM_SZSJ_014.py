...
class YW_GEM_REGISTERMM_SZSJ_014(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GEM_REGISTERMM_SZSJ_014')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GEM_REGISTERMM_SZSJ_014(self):
        title = '即成剩撤买-废单再撤单(OMS)'
        # 定义当前测试用例的期待值
