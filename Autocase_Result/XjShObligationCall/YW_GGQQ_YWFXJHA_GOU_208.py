...
class YW_GGQQ_YWFXJHA_GOU_208(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFXJHA_GOU_208')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_YWFXJHA_GOU_208(self):
        title = '买平（义务方平仓）：FOK限价全成或撤销-验资（可用资金刚好）（下单金额<费用&&可用资金=（费用-下单金额））'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('10001319', '1', '*', '1', '0', 'C', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
