...
class YW_GGQQ_YWFXJHA_GOU_214(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFXJHA_GOU_214')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_YWFXJHA_GOU_214(self):
        title = '买平（义务方平仓）：FOK限价全成或撤销-验券（持仓刚好）（下单数量=可卖持仓）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('10001317', '1', '*', '1', '0', 'C', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
