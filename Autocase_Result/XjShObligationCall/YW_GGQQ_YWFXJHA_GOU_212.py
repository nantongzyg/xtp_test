...
class YW_GGQQ_YWFXJHA_GOU_212(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFXJHA_GOU_212')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_YWFXJHA_GOU_212(self):
        title = '买平（义务方平仓）：FOK限价全成或撤销-验资（可用资金<=0且下单导致可用资金增加后变成正数）'
        # 定义当前测试用例的期待值
