...
class YW_GGQQ_YWFXJHA_GOU_216(xtp_test_case):
    # YW_GGQQ_YWFXJHA_GOU_216

    def test_YW_GGQQ_YWFXJHA_GOU_216(self):
        title='买平（义务方平仓）：FOK限价全成或撤销-错误的市场'
        #定义当前测试用例的期待值
        ...
        case_goal = {
            '期望状态': '废单',
            'errorID': 11000108,
            'errorMSG': queryOrderErrorMsg(11000108),
            '是否生成报单': '是',
            '是否是撤废': '否',
            'xtp_ID': 0,
            'cancel_xtpID': 0,
        ...
        stkparm = QueryStkPriceQty('999999', '1', '*', '1', '0', 'C', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] is False:
            ...
            wt_reqs = {
                ...
                'order_client_id':1,
                'market': Api.const.XTP_MARKET_TYPE['XTP_MKT_INIT'],
                'ticker': stkparm['证券代码'],
                'side': Api.const.XTP_SIDE_TYPE['XTP_SIDE_BUY'],
                'position_effect':Api.const.XTP_POSITION_EFFECT_TYPE['XTP_POSITION_EFFECT_CLOSE'],
                'price_type': Api.const.XTP_PRICE_TYPE['XTP_PRICE_LIMIT_OR_CANCEL'],
                'price': stkparm['随机中间价'],
                'quantity': 2
            }
