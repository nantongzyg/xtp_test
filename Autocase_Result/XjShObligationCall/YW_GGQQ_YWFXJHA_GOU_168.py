...
class YW_GGQQ_YWFXJHA_GOU_168(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFXJHA_GOU_168')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_YWFXJHA_GOU_168(self):
        title = '卖开（义务方开仓）：FOK限价全成或撤销-验资（可用资金<=0）'
        # 定义当前测试用例的期待值
