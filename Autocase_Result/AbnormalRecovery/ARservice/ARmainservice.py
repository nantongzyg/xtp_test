...
def service_insertorder(Api,wt_reqs,config):
    ...
    if xtp_ID == 0:
        ...
        #logger.warning('''异常恢复前订单信息：%s'''%str(order_info))
...
def service_cancleorder(Api,wt_reqs,config):
    ...
    if xtp_ID == 0:
        ...
        #logger.warning('''异常恢复前撤单订单信息：%s'''%str(order_info))
...
def service_restart(Api,config):
    ...
    for xtpid in restart_info[username].keys():
...
def query_capital_stock(Api,orders,config,ticker):
    ...
    #logger.warning('''资金信息：%s'''%str(orders))
...
def check_result(order_info, restart_info):
    ...
    if False:
        ...
        logger.warning('''异常恢复前订单信息：%s'''%str(order_info))
        logger.warning('''异常恢复后订单信息：%s'''%str(restart_info))
    return result
