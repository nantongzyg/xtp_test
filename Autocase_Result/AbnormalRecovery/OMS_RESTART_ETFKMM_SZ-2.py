...
'''
class OMS_RESTART_ETFKMM_SZ(unittest.TestCase):

    def test_OMS_RESTART_ETFKMM_SZ(self):

        title = '异常恢复：重启OMS-跨市场ETF买卖-深圳'
        logger.warning(title)
        # 定义委托参数信息------------------------------------------
        # 参数：证券代码、市场、证券类型、证券状态、交易状态、买卖方向（Ｂ买Ｓ卖）、期望状态、Api
        '''
        stkparm = QueryStkPriceQty('159919', '2', '15', '2', '0', 'B', '全成',
                                   tradeApi)
        # 如果下单参数获取失败，则用例失败
        ...
        for user in ALL_USER:
            ...
            service_restart(Api,user)
                # 查询当前用户的资金和持仓
            query_capital_stock(Api,restart_info ,user,'000100')
