...
class sj_put_init(xtp_test_case):
    # YW_GGQQ_YWFXJHA_024

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFSJHA_GU_008')
        #clear_data_and_restart_sh()
        all_restart()
	Api.trade.Logout()
        Api.trade.Login()

    def test_sj_put_init(self):
        title = '卖开（义务方开仓）：T日增加20002380和20002378空头持仓'
        # 定义当前测试用例的期待值
        ...
        for i in xrange(3):
            for code, quantity in {'20002380': 100, '20002378': 10000}.items():
                stkparm = QueryStkPriceQty(code, '1', '*', '1', '0', 'P', case_goal['期望状态'], Api)
                # 如果下单参数获取失败，则用例失败
