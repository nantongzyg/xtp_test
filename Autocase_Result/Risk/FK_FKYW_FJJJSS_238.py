...
class FK_FKYW_FJJJSS_238(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.delete_cur_risk()
        sql_transfer.insert_cur_risk('FK_FKYW_GPWT_626')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        time.sleep(2)
        Api.trade.Login()

    def test_FK_FKYW_FJJJSS_238(self):
        title = '默认rule26,rule0=0'
        #定义当前测试用例的期待值
        ...
        if stkparm['返回结果'] == False:
            ...
            count = 1
            max_count = 501
            filename = 'FK_FKYW_FJJJSS_238_order'
