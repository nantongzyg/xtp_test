...
class FK_FKYW_ETFWT_628_K(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.delete_cur_risk()
        sql_transfer.insert_cur_risk('FK_FKYW_GPWT_628')
        oms_restart()
        Api.trade.Logout()
        time.sleep(2)
        Api.trade.Login()

    def test_FK_FKYW_ETFWT_628_K(self):
        title = '默认rule27,rule0=0'
        #定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('999999','2','15','2','0','B',case_goal['期望状态'],Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] == False:
            ...
            count = 1
            max_count = 20
            filename = 'FK_FKYW_GPWT_628_K_order'
