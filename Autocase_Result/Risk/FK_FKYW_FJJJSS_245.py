...
class FK_FKYW_FJJJSS_245(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.delete_cur_risk()
        sql_transfer.insert_cur_risk('FK_FKYW_GPWT_632')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        time.sleep(2)
        Api.trade.Login()

    def test_FK_FKYW_FJJJSS_245(self):
        title = '默认rule43,rule0=0'
        #定义当前测试用例的期待值
        ...
        if stkparm['返回结果'] is False:
            ...
            count = 1
            max_count = 5000
            filename1 = 'FK_FKYW_FJJJSS_245_order1'
            filename2 = 'FK_FKYW_FJJJSS_245_order2'
