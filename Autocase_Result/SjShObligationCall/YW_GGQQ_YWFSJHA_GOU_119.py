...
class YW_GGQQ_YWFSJHA_GOU_119(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFSJHA_GOU_119')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_YWFSJHA_GOU_119(self):
        title = '卖开（义务方开仓）：市价剩余转撤销-验资（可用资金<0，下单后导致可用资金增加，但是依然为负数）'
        # 定义当前测试用例的期待值
