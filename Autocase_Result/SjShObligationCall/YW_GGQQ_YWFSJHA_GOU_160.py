...
class YW_GGQQ_YWFSJHA_GOU_160(xtp_test_case):

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFSJHA_GOU_160')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_GGQQ_YWFSJHA_GOU_160(self):
        title = '买平（义务方平仓）：市价剩余撤销-验券（持仓不足）（下单数量>可卖持仓）'
        # 定义当前测试用例的期待值
