...
class sj_call_init(xtp_test_case):
    # YW_GGQQ_YWFXJHA_024

    def setUp(self):
        sql_transfer = Opt_SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_GGQQ_YWFSJHA_GOU_008')
        #clear_data_and_restart_sh()
        #print '-'*30
        #Api.trade.Logout()
        #Api.trade.Login()

    def test_sj_call_init(self):
        ...
        for i in xrange(2):
            for code, quantity in {'11002633': 100, '20002438': 10000}.items():
                stkparm = QueryStkPriceQty(code, '1', '*', '1', '0', 'C', case_goal['期望状态'], Api)
                # 如果下单参数获取失败，则用例失败
