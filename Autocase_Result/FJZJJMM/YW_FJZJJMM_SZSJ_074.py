...
class YW_FJZJJMM_SZSJ_074(xtp_test_case):
    # YW_FJZJJMM_SZSJ_074
    def test_YW_FJZJJMM_SZSJ_074(self):
        title = '交易日即成剩撤-T+0买→T+0卖（未持有证券代码）'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('151186', '2', '24', '2', '0', 'B', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        ...
        # T+0买
        title = '深圳Ａ股交易日即成剩撤（未持有证券代码）-T+0买'
        ...
        self.assertEqual(rs['用例测试结果'], True)
        # T+0卖
        title = '深圳Ａ股交易日即成剩撤（未持有证券代码）-T+0卖'
        logger.warning(title)
