...
class YW_FJZJJMM_SZSJ_029(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_FJZJJMM_SZSJ_029')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_FJZJJMM_SZSJ_029(self):
        title = '默认1：可用资金正好-深A即成剩撤买（可用资金=下单金额+费用）'
        # 定义当前测试用例的期待值
        # 期望状态：初始、未成交、部成、全成、部撤已报、部撤、已报待撤、已撤、废单、撤废、内部撤单
        # xtp_ID和cancel_xtpID默认为0，不需要变动
        case_goal = {
            '期望状态': ['全成','部撤'][trade_type_allpart],
            'errorID': 0,
            'errorMSG': '',
            '是否生成报单': '是',
            '是否是撤废': '否',
        ...
        stkparm = QueryStkPriceQty('151130', '2', '24', '2', '0', 'B', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
