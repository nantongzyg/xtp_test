...
class YW_KCB_HASPRICELIMIT_GPMM_SHSJ_WDZX_136(xtp_test_case):
    def setUp(self):
    ...
    # YW_KCB_HASPRICELIMIT_GPMM_SHSJ_WDZX_136
    def test_YW_KCB_HASPRICELIMIT_GPMM_SHSJ_WDZX_136(self):
        title = '交易日五档即成转限价-T+0买→T+0卖（T-1日未持有证券代码688000)'
        # 定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('688009', '1', '4', '2', '0', 'B', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果'] is False:
            ...
            #self.assertEqual(rs['用例测试结果'], True) # 219
