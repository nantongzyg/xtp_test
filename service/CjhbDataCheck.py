    ...
    if (wt_reqs['side'] == Api.const.XTP_SIDE_TYPE['XTP_SIDE_BUY'] or \
        ...
        if xtp_id == hb_data['order_xtp_id'] and wt_reqs['market'] == hb_data['market'] and wt_reqs['ticker'] == \
            ...
            if  abs(hb_data['trade_amount'] - round(hb_data['price'] * hb_data['quantity'],2)) < 0.01:
                #a=hb_data['trade_amount'] 
                #b=round(hb_data['price'] * hb_data['quantity'],2)
                #print 'aaa',a,b,a-b
                ...
                #print hb_data['price'],hb_data['quantity'],hb_data['trade_amount']
                cjhb_reslut['成交回报检查状态'] = 'end'
                cjhb_reslut['测试结果'] = False
                ...
                # 全部成交－－－－－－－－－－－－－
                    ...
                    if expectStatus == '全成':
                        ...
                        if wt_reqs['business_type'] == Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_ALLOTMENT']:
                            ...
                            CJHB_QC_S(QueryInit, QueryEnd, wt_reqs, etf_sell_flag, upPrice)
                    elif expectStatus == '部成':
                        logger.error('成交回报检查：错误,期待状态为部成，但成交回报数量累计已经全部成交！')
                        cjhb_reslut['成交回报检查状态'] = 'end'
                        cjhb_reslut['测试结果'] = False
...
def CJHB_QC_B(QueryInit,QueryEnd,wt_reqs,upPrice):
    ...
    if wt_reqs['business_type'] == Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_REPO']:
        ...
        fee = max(fee_min,cjhb_reslut['成交金额'] * fee_rate)
    ...
    #--判断可用股份数是否正确(可转债为T+0交易，其他业务为T+1交易)
    elif getSecurityType(wt_reqs['ticker']) == 8 and abs(QueryEnd['可用股份数'] - QueryInit['可用股份数'] - cjhb_reslut['成交数量']) > 0.00001:
        logger.error('可用股份数不正确，预期可用股份数，实际可用股份数分别是'+str(QueryInit['可用股份数']+cjhb_reslut['成交数量'])+','+str(QueryEnd['可用股份>数']))
        cjhb_reslut['成交回报检查状态'] = 'end'
        cjhb_reslut['测试结果'] = False
        cjhb_reslut['测试错误原因'] = ['可用股份数不正确', {
            '原可用股份数': QueryInit['可用股份数'],
    ...
    elif getSecurityType(wt_reqs['ticker']) != 8 and QueryEnd['可用股份数'] !=QueryInit['可用股份数']:
        logger.error('可用股份数不正确，原可用股份数，现可用股份数分别是'+str(QueryInit['可用股份数'])+','+str(QueryEnd['可用股份数']))
        cjhb_reslut['成交回报检查状态'] = 'end'
        cjhb_reslut['测试结果'] = False
        cjhb_reslut['测试错误原因'] = ['可用股份数不正确', {
           '原可用股份数': QueryInit['可用股份数'],
...
def CJHB_BC_B(QueryInit,wt_reqs,QueryEnd,upPrice):
    ...
    if wt_reqs['business_type'] == Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_REPO']:
        ...
        price_avg = QueryInit['持仓成本']
        wt_price = 100
        fee = max(fee_min, wt_price * wt_reqs['quantity'] * fee_rate) 
        fee = float(Decimal(Decimal(str(fee)).quantize(Decimal('.01'), rounding=ROUND_HALF_UP)))  
    elif wt_reqs['business_type'] == Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_ALLOTMENT']:
        fee = 0
    else:
        if wt_reqs['price_type'] == Api.const.XTP_PRICE_TYPE['XTP_PRICE_LIMIT']:
	    #深市债券预扣资金价格第3位小数非零进一位
            secuid=str(QuerySecuid(wt_reqs['ticker']))
            market = str(wt_reqs['market'])
            print 'wt_price = ',wt_reqs['price']
            if secuid in ('5','6','7','8') and market == 'XTP_MKT_SZ_A' and wt_reqs['price']*1000%10 != 0:
            ...
            # 1.1.19.17版本-现货买卖计算预扣资金时考虑超出涨跌幅范围的价格。
    ...
    if abs(QueryInit['可用资金'] - cjhb_reslut['成交金额'] - fee - nomatch_qty * wt_price - QueryEnd['可用资金']) > 0.00001:
        a=QueryInit['可用资金'] - cjhb_reslut['成交金额'] - fee - nomatch_qty * wt_price - QueryEnd['可用资金']
        #print a 
...
def CJHB_QC_S(QueryInit,QueryEnd,wt_reqs,etf_sell_flag,upPrice):
    ...
    if abs(QueryInit['可用资金'] + cjhb_reslut['成交金额'] - fee - QueryEnd['可用资金']) > 0.01:
        a=QueryInit['可用资金'] + cjhb_reslut['成交金额'] - fee - QueryEnd['可用资金']
        print '可用资金',QueryInit['可用资金'],'成交金额',cjhb_reslut['成交金额'],'fee',fee,'end',QueryEnd['可用资金']
        #print a
        logger.error('可用资金计算有错,原可用资金、成交金额、费用、现可用资金分别是'+str(QueryInit['可用资金'])+','+str(cjhb_reslut['成交金额'])+','+str(fee)+','+str(QueryEnd['可用资金']))
        cjhb_reslut['成交回报检查状态'] = 'end'
        cjhb_reslut['测试结果'] = False
...
def CJHB_BC_S(QueryInit,wt_reqs,QueryEnd,upPrice,etf_sell_flag):
    ...
    if wt_reqs['business_type'] == Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_REPO']:
        wt_price = ServiceConfig.Reverse_price
        fee_rate = fee_rate_reverse_repo
        fee = fee_reverse_repo_min
        price_avg = QueryInit['持仓成本']
        fee = max(fee_min, 100 * wt_reqs['quantity'] * fee_rate)
        fee = float(Decimal(Decimal(str(fee)).quantize(Decimal('.01'), rounding=ROUND_HALF_UP)))
    else:
        fee = getFee(wt_reqs, upPrice)
...
def getFee(wt_reqs, upPrice):
    global  cjhb_reslut, fee_rate_buy_special,  fee_rate_sell_special, fee_min_buy_special, fee_min_sell_special,fee_rate_buy_fixed,fee_rate_sell_fixed,fee_addition_buy_fixed,fee_addition_sell_fixed
    secuid=str(QuerySecuid(wt_reqs['ticker']))
    if secuid in ('14','15','17','19') and wt_reqs['side']in (1,2):
    ...
    # 如果是卖，卖出委托不再预扣交易费用，收到成交回报时再扣除交易费用 
        ...
        fee_special = float(Decimal(Decimal(str(fee_special)).quantize(Decimal('.01'), rounding=ROUND_HALF_UP)))
        fee = fee_fixed + fee_special
        if fee > cjhb_reslut['成交金额']:
            fee = cjhb_reslut['成交金额']
    else:
        logger.error("买卖方向错误")
    return fee
