...
from decimal import *
from  CjhbDataCheck import getFee
...
def fundStkCheck_partCancel(Api,QueryInit,QueryEnd,wt_reqs,data):
    ...
    if wt_reqs['side'] == Api.const.XTP_SIDE_TYPE['XTP_SIDE_BUY'] or wt_reqs['business_type'] == Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_REPO']:
        ...
        rs = partCancel_B(QueryInit, QueryEnd, cccb, fee,data, wt_reqs)
        fundStkCheck_reslut['检查状态'] = rs['检查状态']
        fundStkCheck_reslut['测试结果'] = rs['测试结果']
        fundStkCheck_reslut['错误信息'] = rs['错误信息']
    # 如果是卖
...
def nomatch_B(QueryInit,QueryEnd,wt_reqs,Prefreezing,Api):
    ...
    #判断可用资金
        ...
        print QueryInit['可用资金'],Prefreezing,QueryEnd['可用资金']
        logger.error('可用资金预冻结不正确')
        Check_reslut['检查状态']='end'
...
def partCancel_B(QueryInit, QueryEnd, cccb, fee,data,wt_reqs):
    Check_reslut = {
        '检查状态': 'init',
        '测试结果': False,
        '错误信息': '',
    ...
    elif getSecurityType(wt_reqs['ticker']) == 8 and abs(QueryEnd['可用股份数'] - QueryInit['可用股份数'] - data['qty_traded']) > 0.00001:
        logger.error('可用股份数不正确，预期可用股份数，实际可用股份数分别是'+str(QueryInit['可用股份数']+data['qty_traded'])+','+str(QueryEnd['可用股份>数']))
        Check_reslut['成交回报检查状态'] = 'end'
        Check_reslut['测试结果'] = False
        Check_reslut['测试错误原因'] = ['可用股份数不正确', {
