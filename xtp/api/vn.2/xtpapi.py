...
from xtp.api.config import Config
...
class XTPQuoteApi(QuoteApi):
    ...
    def __init__(self, client_id):
        ...
        self.unSubAllOptionMarketDataHandle = lambda data: data
...
class XTPTradeApi(TradeApi):
    ...
    def __init__(self):
        ...
        self.queryCreditDebtInfoHandle = lambda data, error, request_id, is_last: data
        self.queryCreditAssetDebtInfoHandle = lambda data, error, request: data

        self.request_id = 0
        self.session_id = 0
        self.CreateTradeApi(Config.trade.client_id, Config.trade.save_file_path)
        self.SetSoftwareKey(Config.trade.key)
        self.SetSoftwareVersion('1.0')
    ...
    # ----------------------------------------------------------------------
    def onQueryCreditDebtInfo(self, data, error, request_id, is_last):
        """请求查询信用账户负债信息的响应"""
        if self.queryCreditDebtInfoHandle:
            self.queryCreditDebtInfoHandle(data, error, request_id, is_last)

    # ----------------------------------------------------------------------
    def onQueryCreditAssetDebtInfo(self, data, error, request_id):
        """请求查询信用账户待还资金信息的响应"""
        if self.queryCreditAssetDebtInfoHandle:
            self.queryCreditAssetDebtInfoHandle(data, error, request_id)

    # ----------------------------------------------------------------------
    def onQueryCreditTickerAssignInfo(self, data, error, request_id, is_last):
        """请求查询账户可融券头寸信息的响应"""
        if self.queryCreditTickerAssignInfoHandle:
            self.queryCreditTickerAssignInfoHandle(data, error, request_id, is_last)
    ...
    # -----------------------------------------------------------------------
    def QueryCreditDebtInfo(self, request_id=0):
        """请求查询信用账户负债合约信息"""
        if request_id == 0:
            request_id = self.getRequestId()
        return super(XTPTradeApi, self).QueryCreditDebtInfo(self.session_id, request_id)

        # -----------------------------------------------------------------------
    def QueryCreditAssetDebtInfo(self, request_id=0):
        """请求查询信用账户负债合约信息"""
        if request_id == 0:
            request_id = self.getRequestId()
        return super(XTPTradeApi, self).QueryCreditAssetDebtInfo(self.session_id, request_id)
    ...
    def QueryOrdersSync(self, query, request_id=0):
        ...
        if request_id == 0:
            request_id = self.getRequestId()
        return pretty_data_query_orders(super(XTPTradeApi, self).QueryOrdersSync(query, self.session_id, request_id))
    ...
    def QueryCreditFundInfoSync(self, request_id=0):
        """请求查询信用账户信息"""
        if request_id == 0:
            request_id = self.getRequestId()
        return pretty_data(
            super(XTPTradeApi, self).QueryCreditFundInfoSync(self.session_id, request_id))

    def QueryCreditTickerDebtInfoSync(self, request_id=0):
        """请求查询信用账户信息"""
        if request_id == 0:
            request_id = self.getRequestId()
        return pretty_data(
            super(XTPTradeApi, self).QueryCreditTickerDebtInfoSync(self.session_id, request_id))

    def QueryCreditDebtInfoSync(self, request_id=0):
        """请求查询信用账户信息"""
        if request_id == 0:
            request_id = self.getRequestId()
        return pretty_data2(
            super(XTPTradeApi, self).QueryCreditDebtInfoSync(self.session_id, request_id))

    def QueryCreditAssetDebtInfoSync(self, request_id=0):
        """请求查询信用账户信息"""
        if request_id == 0:
            request_id = self.getRequestId()
        return pretty_data(
            super(XTPTradeApi, self).QueryCreditAssetDebtInfoSync(self.session_id, request_id))

    def QueryCreditTickerAssignInfoSync(self, query, request_id=0):
        """请求查询账户可融券头寸信息"""
        if request_id == 0:
            request_id = self.getRequestId()

        return pretty_data(
            super(XTPTradeApi, self).QueryCreditTickerAssignInfoSync(query, self.session_id, request_id))
    ...
    def setQueryCreditDebtInfoHandle(self, func):
        self.queryCreditDebtInfoHandle = func

    def setQueryCreditAssetDebtInfoHandle(self, func):
        self.queryCreditAssetDebtInfoHandle = func

    def setQueryCreditTickerAssignInfoHandle(self, func):
        self.queryCreditTickerAssignInfoHandle = func
