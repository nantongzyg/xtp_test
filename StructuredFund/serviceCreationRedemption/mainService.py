...
def serviceTest(Api,case_goal,wt_reqs, py_name = ''):
    # ------------------------------------------------------------------------------------------------------------------
    ...
    def on_QueryOrder(data,error,reqID,is_last):
        ...
        if bdQuecyCheck['flag'] and bdQuecyCheck['检查状态']=='end':
            rs=BdqueryDataCheck(Api,wt_reqs,case_goal,data,error,QueryInit,QueryEnd,py_name)
            if rs['报单类型']=='报单':
                bdquery_rs=rs
    ...
    # ------------------------------------------------------------------------------------------------------------------
    if case_goal['期望状态'] in ('未成交', '已撤', '废单'):
