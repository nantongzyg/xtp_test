...
#报单查询结果业务处理方法
def BdqueryDataCheck(Api,wt_reqs,case_goal,data,error,QueryInit,QueryEnd,py_name):
    # 定义测试结果参数
    result = {
        '报单检查状态': 'init',
        '测试结果': False,
    ...
    if data['order_submit_status'] in (
        ...
        if public_result['报单检查状态'] == 'pending' and public_result['测试结果'] == True:
            bdQuery_result = bd_type_Handle(Api, wt_reqs, case_goal, data, error, QueryInit, QueryEnd, py_name)
...
def public_Handle(Api,wt_reqs,data,case_goal):
    ...
    elif data['price_type'] != Api.const.XTP_PRICE_TYPE['XTP_PRICE_LIMIT']:
        logger.error('错误，报单应该返回的价格条件和实际返回的价格条件不一致，应返回和实际返回价格条件分别是' +
                     str(Api.const.XTP_PRICE_TYPE['XTP_PRICE_LIMIT']) + ',' +
                     str(data['price_type']))
        public_result['报单检查状态'] = 'end'
        public_result['测试结果'] = False
        public_result['错误信息'] = '报单返回的价格条件错误'
...
#--报单为报单类型的业务处理
#-----------------------------------------------------------------------------------------------------------------------
def bd_type_Handle(Api,wt_reqs,case_goal,data,error,QueryInit,QueryEnd,py_name):
    # 定义测试结果参数
    bdQuery_result = {
        '报单检查状态': 'init',
        '测试结果': False,
    ...
    # --如果报单查询是‘未成交’状态的执行如下
        ...
        if data['order_submit_status'] == Api.const.XTP_ORDER_SUBMIT_STATUS_TYPE[
            ...
            if case_goal['期望状态'] == '未成交':
                # 检查资金持仓是否正确
                fundstkcheck_rs = fundStkCheck_nomatch(Api, QueryInit, QueryEnd, wt_reqs, case_goal, py_name)
