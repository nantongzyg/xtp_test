...
#---定义费率，最低收费
fee_rate_creation = ServiceConfig.FEE_RATE_STRUCTURED_FUND_CREATION
fee_rate_redemption = ServiceConfig.FEE_RATE_STRUCTURED_FUND_REDEMPTION
fee_min = ServiceConfig.FEE_MIN_STRUCTURED_FUND_CREATION_REDEMPTION

#报单状态为'初始'、'未成交'，资金股份校验
def fundStkCheck_nomatch(Api,QueryInit,QueryEnd,wt_reqs,case_goal,py_name):
    ...
    rs = nomatch_bs(Api, QueryInit, QueryEnd, wt_reqs, py_name)
...
def nomatch_bs(Api,QueryInit,QueryEnd,wt_reqs,py_name):
    global fee_rate_creation, fee_rate_redemption, fee_min

    Prefreezing = 0
    # 申购
    # 获取ServiceConfig.py中某些case测试时的费用设置

    if py_name != '':
        fee_fund = ServiceConfig.fee_structured_fund_creation_redemption[py_name]
        fee_min = float(fee_fund[ServiceConfig.fee_min_structured_fund_creation_redemption_str])
        fee_rate_creation = float(fee_fund[ServiceConfig.fee_rate_structured_fund_creation_str])
        fee_rate_redemption = float(fee_fund[ServiceConfig.fee_rate_structured_fund_redemption_str])
    if wt_reqs['side'] == Api.const.XTP_SIDE_TYPE['XTP_SIDE_PURCHASE']:
        if wt_reqs['quantity'] * fee_rate_creation >= fee_min:
            fee = wt_reqs['quantity'] * fee_rate_creation
        else:
            fee = fee_min
        Prefreezing = wt_reqs['quantity'] + fee
        rs = nomatch_B(QueryInit, QueryEnd, wt_reqs, Prefreezing, Api)
    # 赎回
    elif wt_reqs['side'] == Api.const.XTP_SIDE_TYPE['XTP_SIDE_REDEMPTION']:
        nav = float(QueryTable(fund_creation,
                         ['nav'],
                         {'instrument_id': wt_reqs['ticker']},
                         2)['nav']) / 10000
        if wt_reqs['quantity'] * fee_rate_redemption * nav >= fee_min:
            fee = wt_reqs['quantity'] * fee_rate_redemption * nav
        else:
            fee = fee_min
        Prefreezing = fee
        rs = nomatch_S(QueryInit, QueryEnd, wt_reqs, Prefreezing, Api)
    else:
...
def nomatch_S(QueryInit, QueryEnd, wt_reqs, Prefreezing, Api):
    Check_reslut = {
        '检查状态': 'init',
        '测试结果': False,
        '错误信息': '',
    ...
    # 判断昨日持仓是否正确
        ...
        Check_reslut['错误信息'] = '昨日持仓计算不正确'
    # 判断今日可申购赎回持仓是否正确
    elif QueryInit['今日可申购赎回持仓'] - wt_reqs['quantity'] != QueryEnd['今日可申购赎回持仓']:
        logger.error('今日可申购赎回持仓计算不正确')
        Check_reslut['检查状态'] = 'end'
        Check_reslut['错误信息'] = '今日可申购赎回持仓计算不正确'
    else:
