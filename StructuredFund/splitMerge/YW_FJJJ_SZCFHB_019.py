...
class YW_FJJJ_SZCFHB_019(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_FJJJ_SZCFHB_019')
        oms_restart()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_FJJJ_SZCFHB_019(self):
        title='深圳分级基金拆分合并—分拆--验资验券：资金=0'
        #定义当前测试用例的期待值
