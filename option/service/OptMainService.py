...
from OptCjhbDataCheck import *
from OptBdtsDataCheck import *
from OptBdqueryDataCheck import *
from OptCaseEndCheck import *
from OptCancelOrderErrorDataCheck import *
...
def serviceTest(Api,case_goal,wt_reqs):
    # ------------------------------------------------------------------------------------------------------------------
    ...
    def on_QueryOrder(data,error,reqID,is_last):
        ...
        elif bdQuecyCheck['检查状态']=='end' and bdQuecyCheck['flag'] is False:
            bdquery_rs={
                '检查状态':'end',
                '测试结果':False,
    ...
    def on_order(data, error):
        ...
        elif bdCheck['status']=='pending' and case_goal['期望状态'] != '未成交' and case_goal['是否是撤废']=='是':
...
def setQueryInit(Api,wt_reqs):
    # 资金查询及赋值QueryInit-------------------------------------
    fundasset0 = Api.trade.QueryAssetSync()['data']
    #print fundasset0
    if fundasset0 == {}:
        logger.error('未查到资金持仓数据，请检查测试环境')
    elif fundasset0['is_last']:
        QueryInit['总资产'] = fundasset0['total_asset']
        QueryInit['可用资金'] = fundasset0['buying_power']
        QueryInit['证券资产'] = fundasset0['security_asset']
        QueryInit['买入资金'] = fundasset0['fund_buy_amount']
        QueryInit['买入费用'] = fundasset0['fund_buy_fee']
        QueryInit['卖出资金'] = fundasset0['fund_sell_amount']
        QueryInit['卖出费用'] = fundasset0['fund_sell_fee']
        QueryInit['预扣资金'] = fundasset0['withholding_amount']
        QueryInit['冻结保证金'] = fundasset0['frozen_margin']
        QueryInit['行权冻结资金'] = fundasset0['frozen_exec_cash']
        QueryInit['行权费用'] = fundasset0['frozen_exec_fee']
        QueryInit['垫付资金'] = fundasset0['pay_later']
        QueryInit['预垫付资金'] = fundasset0['preadva_pay']
        QueryInit['昨日余额'] = fundasset0['orig_banlance']
        QueryInit['当前余额'] = fundasset0['banlance']
        QueryInit['当天出入金'] = fundasset0['deposit_withdraw']
        QueryInit['当日交易资金扎差'] = fundasset0['trade_netting']
        QueryInit['资金资产'] = fundasset0['captial_asset']
    else:
        logger.error("初始资金查询：查询的资金返回值非is_last")

    # 持仓查询及赋值QueryInit------------------------------------------
    QueryInit['证券代码'] = wt_reqs['ticker']
    ...
    direction_type = getPositionDirection(wt_reqs)

    stkcode = {
        'ticker': ''
    }
    stkasset0 = Api.trade.QueryPositionListSync(stkcode)
    for item in stkasset0:
        for k, v in item.items():
            if k == wt_reqs['ticker']:
                if direction_type == v['position']['position_direction']:
                    QueryInit['证券代码'] = v['position']['ticker']
                    QueryInit['市场'] = v['position']['market']
                    QueryInit['拥股数量'] = v['position']['total_qty']
                    QueryInit['可用股份数'] = v['position']['sellable_qty']
                    QueryInit['持仓成本'] = v['position']['avg_price']
                    QueryInit['浮动盈亏'] = v['position']['unrealized_pnl']
                    QueryInit['昨日持仓'] = v['position']['yesterday_position']
                    QueryInit['今日可申购赎回持仓'] = v['position']['purchase_redeemable_qty']
                    # QueryInit['持仓方向'] = v['position']['position_side']
                    QueryInit['可行权合约'] = v['position']['executable_option']
                    QueryInit['可锁定标的'] = v['position']['lockable_position']
                    QueryInit['可行权标的'] = v['position']['executable_underlying']
                    QueryInit['已锁定标的'] = v['position']['locked_position']
                    QueryInit['可用已锁定标的'] = v['position']['usable_locked_position']
...
def setQueryEnd(Api,wt_reqs):
    # 查询资金-------------------------------------
    fundasset9 = Api.trade.QueryAssetSync()['data']
    if fundasset9 == {}:
        logger.error('未查到资金持仓数据，请检查测试环境')
    elif fundasset9['is_last']:
        QueryEnd['总资产'] = fundasset9['total_asset']
        QueryEnd['可用资金'] = fundasset9['buying_power']
        QueryEnd['证券资产'] = fundasset9['security_asset']
        QueryEnd['买入资金'] = fundasset9['fund_buy_amount']
        QueryEnd['买入费用'] = fundasset9['fund_buy_fee']
        QueryEnd['卖出资金'] = fundasset9['fund_sell_amount']
        QueryEnd['卖出费用'] = fundasset9['fund_sell_fee']
        QueryEnd['预扣资金'] = fundasset9['withholding_amount']
        QueryEnd['冻结保证金'] = fundasset9['frozen_margin']
        QueryEnd['行权冻结资金'] = fundasset9['frozen_exec_cash']
        QueryEnd['行权费用'] = fundasset9['frozen_exec_fee']
        QueryEnd['垫付资金'] = fundasset9['pay_later']
        QueryEnd['预垫付资金'] = fundasset9['preadva_pay']
        QueryEnd['昨日余额'] = fundasset9['orig_banlance']
        QueryEnd['当前余额'] = fundasset9['banlance']
        QueryEnd['当天出入金'] = fundasset9['deposit_withdraw']
        QueryEnd['当日交易资金扎差'] = fundasset9['trade_netting']
        QueryEnd['资金资产'] = fundasset9['captial_asset']
    else:
        logger.error("收到成交回报资金查询：查询的资金返回值非is_last")

    # 收到成交回报后查询持仓-------------------------------------
    QueryEnd['证券代码'] = wt_reqs['ticker']
    ...
    stkasset9 = Api.trade.QueryPositionListSync(stkcode)
    for item in stkasset9:
        for k, v in item.items():
            if k == wt_reqs['ticker']:
                if direction_type == v['position']['position_direction']:
                    QueryEnd['证券代码'] = v['position']['ticker']
                    QueryEnd['市场'] = v['position']['market']
                    QueryEnd['拥股数量'] = v['position']['total_qty']
                    QueryEnd['可用股份数'] = v['position']['sellable_qty']
                    QueryEnd['持仓成本'] = v['position']['avg_price']
                    QueryEnd['浮动盈亏'] = v['position']['unrealized_pnl']
                    QueryEnd['昨日持仓'] = v['position']['yesterday_position']
                    QueryEnd['今日可申购赎回持仓'] = v['position']['purchase_redeemable_qty']
                    # QueryEnd['持仓方向'] = v['position']['position_side']
                    QueryEnd['可行权合约'] = v['position']['executable_option']
                    QueryEnd['可锁定标的'] = v['position']['lockable_position']
                    QueryEnd['可行权标的'] = v['position']['executable_underlying']
                    QueryEnd['已锁定标的'] = v['position']['locked_position']
                    QueryEnd['可用已锁定标的'] = v['position']['usable_locked_position']
...
def getPositionDirection(wt_reqs):
    side_type = Api.const.XTP_SIDE_TYPE
    effect_type = Api.const.XTP_POSITION_EFFECT_TYPE
    direction_type = Api.const.XTP_POSITION_DIRECTION_TYPE
    if wt_reqs['side'] == side_type['XTP_SIDE_BUY']:
        if wt_reqs['position_effect'] == effect_type['XTP_POSITION_EFFECT_OPEN']:
            return direction_type['XTP_POSITION_DIRECTION_LONG']
        else:
            return direction_type['XTP_POSITION_DIRECTION_SHORT']
    else:
        if wt_reqs['position_effect'] == effect_type['XTP_POSITION_EFFECT_OPEN']:
            return direction_type['XTP_POSITION_DIRECTION_SHORT']
        else:
