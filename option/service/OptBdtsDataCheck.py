...
def public_Handle(Api,wt_reqs,data,case_goal):
    ...
    not_execute = (wt_reqs['business_type'] != Api.const.XTP_BUSINESS_TYPE['XTP_BUSINESS_TYPE_EXECUTE'])
    ...
    # 判断委托方向是否一致
    elif data['side'] != wt_reqs['side'] and not_execute:
    ...
    # 判断开平标志是否一致
    elif data['position_effect'] != wt_reqs['position_effect'] and not_execute:
        logger.error('报单返回的开平标志与原委托不一致')
        public_result['报单检查状态'] = 'end'
        public_result['测试结果'] = False
        public_result['错误信息'] = '报单返回的开平标志与原委托不一致'
    ...
    elif checkDataPrice(Api, wt_reqs, data['price']) is False and not_execute:
        logger.error('报单推送返回的价格验证错误')
    elif data['price_type'] != wt_reqs['price_type'] and not_execute:
        logger.error('错误，报单返回的价格条件与原委托不一致，原委托价格条件和报单回报的价格条件分别是' + str(
            wt_reqs['price_type']) + ',' + str(data['price_type']))
        public_result['报单检查状态'] = 'end'
        public_result['测试结果'] = False
        public_result['错误信息'] = '报单返回的价格条件与原委托不一致'
    else:
...
def bd_type_Handle(Api,wt_reqs,case_goal,data,error):
    ...
    # 判断撤单xtp_ID是否一致
        ...
        bdhb_result['报单检查状态'] = 'end'
        bdhb_result['测试结果'] = False
        bdhb_result['错误信息'] = '报单返回的撤单xtpID与期望不一致'
    ...
    # --如果报单是‘未成交’状态的执行如下，通过下单后的报单推送
        ...
        if data['order_submit_status'] == Api.const.XTP_ORDER_SUBMIT_STATUS_TYPE[
            ...
            logger.info('报单回报状态为未成交,提交状态为已提交,'+str(data['order_status'])+','+str(data['order_submit_status']))
            # 如果期望状态是未成交，执行(‘部成’的报单推送与‘未成交’的一样)
            if case_goal['期望状态'] in('未成交','部成','已报待撤','部撤已报') :
                bdhb_result = no_match(wt_reqs, case_goal, data, error)
            elif case_goal['期望状态'] in('全成','部撤','已撤','废单','撤废'):
    ...
    # --如果报单回报是全成状态的，执行如下
        ...
        if data['order_submit_status'] == Api.const.XTP_ORDER_SUBMIT_STATUS_TYPE[
            ...
            if case_goal['期望状态'] == '全成':
                bdhb_result = all_match(wt_reqs, case_goal, data, error)
    ...
    # --如果报单是部撤状态的，执行如下
        ...
        if data['order_submit_status'] == Api.const.XTP_ORDER_SUBMIT_STATUS_TYPE[
            'XTP_ORDER_SUBMIT_STATUS_INSERT_ACCEPTED']:
            bd_time['部撤'] = time.time()
            logger.info('报单回报状态为部分撤单，'+str(data['order_status'])+','+str(data['order_submit_status']))
            # 期望状态如果是为部撤，执行
            if case_goal['期望状态'] == '部撤':
                bdhb_result = part_cancel(wt_reqs, case_goal, data, error)
