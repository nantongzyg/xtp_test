...
def encode_dict_value(D1):
    ...
    if D1['对象'].encode('utf-8') != '-':
        if int(D1['case_type']) != -1:
            ...
            D2['errorMSG'] = str(D1['errorMSG']).encode('utf-8')
            #添加--作用：按照template.py生成用例时,errid=0的置为'',不为0的,取execl赋值
            D2['errorMSG'] = "''" if D2['errorMSG'] == '' else D2['errorMSG']
            if D2['errorMSG'].endswith('.0'):
                D2['errorMSG'] = D2['errorMSG'][0: -2]
            ...
            if int(D1['case_type']) != 2:
                ...
                D2['security_type'] = D1['security_type'].encode('utf-8')
                D2['security_status'] = str(int(D1['security_status']))
                D2['trade_status'] = str(int(D1['trade_status']))
                D2['call_or_put'] = D1['call_or_put'].encode('utf-8')
    ...
    def gen_specify_casepy(self, pyname,casepath):
        ...
        try:
            ...
            if case_para_tuple[-1] == 0:
                case_str = templet_case_str22 % case_para_tuple
            elif case_para_tuple[-1] == 2:
                case_str = templet_case_str23 % case_para_tuple
            elif case_para_tuple[-1] == 4:
                case_str = templet_case_str24 % case_para_tuple
            else:
                pass
