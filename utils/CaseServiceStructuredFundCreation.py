...
def encode_dict_value(D1):
    ...
    if D1['对象'].encode('utf-8') != '-':
        ...
        if int(D1['case_type']) != 2:
            D2['stkcode'] = str(int(D1['stkcode'])).zfill(6) # 代码补全，不足六位的，左边补0
            ...
            D2['divide_status'] = str(int(D1['divide_status']))
...
class CaseService():
    ...
    def gen_specify_casepy(self, pyname):
        ...
        try:
            ...
            if case_para_tuple[-1] == 0:
                case_str=templet_case_str3 % case_para_tuple
            elif case_para_tuple[-1] == 2:
                case_str = templet_case_str6 % case_para_tuple
            elif case_para_tuple[-1] == 3:
                case_str = templet_case_str5 % case_para_tuple
            elif case_para_tuple[-1] == 5:
                case_str = templet_case_str4 % case_para_tuple
            elif case_para_tuple[-1] == 6:
                case_str = templet_case_str21 % case_para_tuple
            else:
                pass

            # 不生成模板和对象外的case，都不生成自动化py
            if case_para_tuple[-1] != -1 and case_para_dict != {}:
                Autocasefile=os.path.join('../StructuredFund/creationRedemption',pyname+'.py')
