...
class CaseService():
    def __init__(self,excel_file,sheet_name):
        #self.excel_file = u'融资融券异常恢复自动化case参数模板.xlsx'
    ...
    def read_excel(self, excel_file,sheet_name):
        ...
        print testcase_name_para_dict, testcase_seq_dict
    ...
    def gen_specify(self, case_name):
        ...
        print " def gen_specify\n"
        print case_calss
        return case_class

    def gen_specify_casepy(self,excel_file, pyname,casepath):
        print' def gen_specify_casepy'
        ...
        try:
           ...
           # print templet_case_str81
      
            if case_para_tuple[-1] == 0:
                print 'coming'
                print case_para_tuple
                case_str=templet_case_str81 % case_para_tuple
                print "templete-case_str81"
                print case_str
            else:
                pass
            excel_file2=excel_file.replace(".xlsx",'')
            # 不生成模板和对象外的case，都不生成自动化py
            if case_para_tuple[-1] != -1 and case_para_dict != {}:
                Autocasefile=os.path.join('../Autocase_Result/'+excel_file2, casepath,pyname+'.py')
                print Autocasefile
    ...
    def gen_all_casepy(self,excel_file,casepath):
        #生成excel中所有的casename对应的casepy文件
        flag_list=[]
        for key,value in self.testcase_name_para_dict.items():
            pyname=key
            try:
                self.gen_specify_casepy(excel_file,pyname,casepath)
            except:
                pass
