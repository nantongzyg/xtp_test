...
class YW_ETFSS_SZSH_043(xtp_test_case):

    def test_YW_ETFSS_SZSH_043(self):
        # -----------ETF赎回-------------
        title = '深圳ETF赎回--全部成交（数量：最大单位&费用>min，' \
                '上海：99000000；深圳：根据etfbaseinfo中的redemption_limit字段）'
        # 定义当前测试用例的期待值
