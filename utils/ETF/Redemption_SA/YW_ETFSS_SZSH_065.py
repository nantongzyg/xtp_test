...
class YW_ETFSS_SZSH_065(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SZSH_065')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SZSH_065(self):
        # -----------ETF赎回-------------
        title = '深圳：当日赎回的证券，当日可以卖出并检查持仓'
        # 定义当前测试用例的期待值
