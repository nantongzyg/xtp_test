...
class YW_ETFSS_SZSG_047(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SZSG_047')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SZSG_047(self):
        # -----------ETF申购-------------
        title = '深圳ETF申购真实ETF-资金<现金替代所需的资金，申购所需的成分股不足' \
