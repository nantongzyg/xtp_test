...
class YW_ETFSS_SZSG_064_2(xtp_test_case):

    py_name = 'YW_ETFSS_SZSG_064_2'

    def setUp(self):
        sqldata_transfer = SqlData_Transfer()
        sqldata_transfer.update_cur_fee_rate(1, self.py_name)
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SZSG_064_2(self):
        # -----------ETF申购-------------
        title = '深圳ETF申购--交易时间对状态为“全成”进行撤单'
        # 定义当前测试用例的期待值
        ...
        case_goal = {
            ...
            '期望状态': '全成',
            'errorID': 11000343,
            'errorMSG': 'Target order already finished!',
            '是否生成报单': '是',
            '是否是撤废': '是',
            'xtp_ID': 0,
            'cancel_xtpID': 0,
