...
class YW_ETFSS_SZSG_013(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SZSG_013')
        clear_data_and_restart_sz()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SZSG_013(self):
        # -----------ETF申购-------------
        title = '深圳ETF申购--必须全部现金替代：T-1日有成分股可申购1unitETF&可用资金不足→T日申购ETF'
        # 定义当前测试用例的期待值
        ...
        quantity = int(
            unit_info['etf_unit'] * stkparm['最小申赎单位'])  # 二级市场卖出的etf数量
