    ...
    if wt_reqs['side'] == Api.const.XTP_SIDE_TYPE['XTP_SIDE_PURCHASE']:
        ...
        # 资金成交回报校验
            ...
            etf_trade_amount = get_etf_trade_amount(2,
                                                    wt_reqs['ticker'],
                                                    creation_redemption_num)  # etf成交金额
...
def ETF_CJHB_QC_B(QueryInit, QueryEnd, wt_reqs,
    ...
    global fee_etf_min, fee_rate_etf_creation
...
def ETF_CJHB_QC_S(QueryInit, QueryEnd, wt_reqs,
    ...
    :return: None
    '''
    global fee_etf_min, fee_rate_etf_redemption
...
def etf_fund_check_ha(xtp_id, hb_data, wt_reqs, etf_trade_amount):
    ...
    if (xtp_id == hb_data['order_xtp_id'] and
        ...
        cjhb_reslut['测试错误原因'] = '成交回报和委托信息不一致，' \
                                '如：xtpid,market,side'
    # 回报金额小于1000
    elif etf_trade_amount < etf_fund_amount or \
        (etf_trade_amount >= etf_fund_amount and
         hb_data['trade_amount'] < etf_fund_amount):
        trade_amount = etf_trade_amount \
                           if etf_trade_amount < etf_fund_amount \
                           else etf_trade_amount % etf_fund_amount
        # 金额校验
        if abs(hb_data['trade_amount'] - trade_amount) > 0.00001:
        ...
        # 数量校验
            ...
            cjhb_reslut['测试错误原因'] = ('etf资金成交回报-实际成交数量和期望的成交数量不一致，'
                         ...
                         str(etf_fund_quantity_min))
    # 回报金额大于1000
    elif (etf_trade_amount >= etf_fund_amount and \
         hb_data['trade_amount'] >= etf_fund_amount):
        trade_amount = etf_trade_amount - etf_trade_amount % etf_fund_amount
        # 金额校验
        if abs(hb_data['trade_amount'] - trade_amount) > 0.00001:
            logger.error('etf资金成交回报-实际成交金额和期望的成交金额不一致，'
                         '分别为' + str(hb_data['trade_amount']) + ',' +
...
# 计算etf申赎的成交金额
def get_etf_trade_amount(is_creation_redemption, ticker, unit):
    '''
    ...
    components_info = QueryEtfComponentsInfoDB(ticker) # 查询etf成分股申购所需数量、现金替代标识、申赎替代金额
    if is_creation_redemption == 1:
        for stk in components_info:
            # 允许现金替代
            if stk[1] == 1:
                etf_trade_amount += (unit * stk[2] -
                                     components_stk[stk[0]]['今日可申购赎回持仓']) \
                                     * stk[4] * (1 + stk[3])
            # 全部现金替代
            elif stk[1] == 2:
