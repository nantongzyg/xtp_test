...
class YW_ETFSS_SHSH_029(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SHSH_029')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SHSH_029(self):
        # -----------ETF赎回-------------
        ...
        case_goal = {
            ...
            '期望状态': '废单',
            'errorID': 11010125,
            'errorMSG': 'Failed to check amount(include amount received).',
            '是否生成报单': '是',
            '是否是撤废': '否',
            'xtp_ID': 0,
            'cancel_xtpID': 0,
        ...
        unit_info = {
            'ticker': '580620',  # etf代码
            'etf_unit': 5,  # etf赎回单位数
            'etf_unit_sell': 5,  # etf卖出单位数
            'component_unit_sell': 5  # 成分股卖出单位数
        }
