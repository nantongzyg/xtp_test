...
class YW_ETFSS_SHSH_028(xtp_test_case):

    def test_YW_ETFSS_SHSH_028(self):
        # -----------ETF赎回-------------
        title = '赎回真实的ETF-可赎回的证券数不足，资金足额'
        # 定义当前测试用例的期待值
        ...
        case_goal = {
            ...
            '期望状态': '废单',
            'errorID': 11010121,
            'errorMSG': 'Failed to check security quantity.',
            '是否生成报单': '是',
            '是否是撤废': '否',
            'xtp_ID': 0,
            'cancel_xtpID': 0,
        ...
        unit_info = {
            'ticker': '580420',  # etf代码
            'etf_unit': 10,  # etf赎回单位数
            'etf_unit_sell': 0.9,  # etf卖出单位数
            'component_unit_sell': 10  # 成分股卖出单位数
        }
