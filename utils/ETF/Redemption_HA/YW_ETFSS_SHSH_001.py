...
class YW_ETFSS_SHSH_001(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SHSH_001')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SHSH_001(self):
        # -----------ETF赎回-------------
        title = '上海ETF赎回--禁止现金替代：T-1日无ETF持仓→T日赎回ETF'
        # 定义当前测试用例的期待值
