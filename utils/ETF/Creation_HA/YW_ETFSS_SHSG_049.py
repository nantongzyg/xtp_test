...
class YW_ETFSS_SHSG_049(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SHSG_049')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SHSG_049(self):
        # -----------ETF申购-------------
        title = '上海ETF申购真实ETF-资金<必须替代所需的资金，申购所需的成分股充足，' \
                '且禁止现金替代的成分股充足，必须现金替代的成分股也足够多）'
        # 定义当前测试用例的期待值
