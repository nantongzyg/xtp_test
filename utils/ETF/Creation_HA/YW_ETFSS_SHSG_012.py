...
class YW_ETFSS_SHSG_012(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SHSG_012')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SHSG_012(self):
        # -----------ETF申购-------------
        title = '上海ETF申购--必须全部现金替代：T-1日无成分股&可用资金恰好→T日申购ETF'
        # 定义当前测试用例的期待值
        ...
        if stkparm['返回结果'] is False:
            ...
            rs = etfServiceTest(Api, case_goal, wt_reqs, 0)
