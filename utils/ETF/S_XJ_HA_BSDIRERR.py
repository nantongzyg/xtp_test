...
class S_XJ_HA_BSDIRERR(xtp_test_case):

    def test_S_XJ_HA_BSDIRERR(self):
        title='上海逆回购--国债回购--错误的买卖方向'
        #定义当前测试用例的期待值
        ...
        stkparm = QueryStkPriceQty('999999', '1', '12', '2', '0', 'S', case_goal['期望状态'], Api)
        # 如果下单参数获取失败，则用例失败
        if stkparm['返回结果']==False:
            ...
            mainService.ParmIni(Api,case_goal['期望状态'],wt_reqs['price_type'])
            CaseParmInsertMysql(case_goal,wt_reqs)
            rs = mainService.serviceTest(Api, case_goal, wt_reqs)
