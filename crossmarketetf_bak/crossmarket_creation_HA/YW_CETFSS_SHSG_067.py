...
class YW_CETFSS_SHSG_067(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_CETFSS_SHSG_067')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_CETFSS_SHSG_067(self):
        # -----------ETF申购-------------
        title = '上海ETF申购--深市必须全部现金替代：T-1日有成分股可申购1unitETF&可用资金不足→T日申购ETF'
        # 定义当前测试用例的期待值
