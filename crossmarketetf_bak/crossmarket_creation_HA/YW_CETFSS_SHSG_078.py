...
class YW_CETFSS_SHSG_078(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_CETFSS_SHSG_078')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_CETFSS_SHSG_078(self):
        # -----------ETF申购-------------
        title = ('上海ETF申购真实ETF-资金<现金替代所需的资金，申购所需的成分股不足（可现金替代的比例<=最大现金替代比例，且禁止现金替代的成分股充足）')
        # 定义当前测试用例的期待值
        ...
        unit_info = {
            'ticker': '540150',  # etf代码
            'optional_unit': 0.0,  # 买入允许现金替代成分股份数
            'forbidden_unit': 0.0,  # 买入禁止现金替代成分股份数
