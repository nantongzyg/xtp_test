...
class YW_CETFSS_SHSG_001(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_CETFSS_SHSG_001')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_CETFSS_SHSG_001(self):
        # -----------ETF申购-------------
        title = '上海ETF申购--禁止现金替代：T-1日无成分股→T日申购ETF'
        # 定义当前测试用例的期待值
        ...
        wt_reqs = {
            ...
            'quantity':
                unit_info['etf_unit'] * unit_number
        }
        ...
        # --------二级市场，卖出etf-----------
        quantity = unit_info['etf_unit'] * unit_number # 二级市场卖出的etf数量
