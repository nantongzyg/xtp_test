...
class YW_ETFSS_SZSG_044(xtp_test_case):

    def test_YW_ETFSS_SZSG_044(self):
        # -----------ETF申购-------------
        title = '深圳ETF申购真实ETF-申购所需的成分股、资金充足' \
                '（可现金替代的比例<=最大现金替代比例）'
        # 定义当前测试用例的期待值
        ...
        unit_info = {
            ...
            'optional_unit_one': -100,  # 取一个成分股的买入数量为：总数-100，以满足'可现金替代的比例<=最大现金替代比例'
