...
#报单查询结果业务处理方法
def etf_BdqueryDataCheck(Api,wt_reqs,case_goal,data,error,QueryInit,
    ...
    if data['order_submit_status'] in (
        ...
        if public_result['报单检查状态'] == 'pending' and public_result['测试结果'] == True:
            bdQuery_result = etf_bd_type_Handle(Api, wt_reqs, case_goal, data, error,
        ...
        public_result = etf_public_Handle(Api, wt_reqs, data)
        if public_result['报单检查状态'] == 'pending' and public_result['测试结果'] == True:
            cdQuery_result = etf_cd_type_Handle(Api, wt_reqs, case_goal, data, error, QueryInit, QueryEnd, component_stk_info)
...
#--报单为报单类型的业务处理
#-----------------------------------------------------------------------------------------------------------------------
def etf_bd_type_Handle(Api,wt_reqs,case_goal,data,error,
                       QueryInit,QueryEnd,component_stk_info,
                       cash_substitute_amount):
    # 定义测试结果参数
    bdQuery_result = {
    ...
    # -如果报单查询是‘初始’状态的执行如下
        ...
        if data['order_submit_status'] == Api.const.XTP_ORDER_SUBMIT_STATUS_TYPE[
            ...
            if case_goal['期望状态'] == '初始':
                ...
                # 现金替代金额
                if wt_reqs['side'] == Api.const.XTP_SIDE_TYPE[
                    'XTP_SIDE_PURCHASE']:
                    trade_fund = (cash_substitute['creation_cash_substitute'] *
                                  wt_reqs[
                                      'quantity'] / creation_redemption_unit / 10000 +
                                  cash_substitute_amount)
                else:
                    trade_fund = (cash_substitute['creation_cash_substitute'] *
                                  wt_reqs[
                                      'quantity'] / creation_redemption_unit / 10000)
                # 检查资金持仓是否正确
                fundstkcheck_rs = fundStkCheck_init(Api, QueryInit, QueryEnd,
                                                    wt_reqs, trade_fund, component_stk_info)
    ...
    # --如果报单查询是‘未成交’状态的执行如下
        ...
        if data['order_submit_status'] == \
            ...
            logger.info('报单查询状态为未成交,提交状态为已提交，' + str(
            ...
            if case_goal['期望状态'] == '未成交':
                ...
                # 检查资金持仓是否正确
                fundstkcheck_rs = fundStkCheck_nomatch(Api, QueryInit,
                                                       QueryEnd, wt_reqs,
                                                       trade_fund)
                # 如果资金持仓检查正确，则执行报单数据检查，否则结束
                if fundstkcheck_rs['检查状态'] == 'end' and \
                    fundstkcheck_rs['测试结果'] == True:
                    # 报单数据检查
                    rs = no_match(wt_reqs, case_goal, data, error, trade_fund)
                    bdQuery_result['报单检查状态'] = rs['报单检查状态']
                    bdQuery_result['测试结果'] = rs['测试结果']
                    bdQuery_result['错误信息'] = rs['错误信息']
                else:
            ...
            elif case_goal['期望状态'] in ('全成','废单'):
                bdQuery_result['报单检查状态'] = 'pending'
                bdQuery_result['测试结果'] = False
                bdQuery_result['错误信息'] = ''
            else:
                logger.error('期望状态和报单状态、提交状态不匹配，期望状态、报单状态、提交状态分别为' + str(
                    case_goal['期望状态']) + ',' + str(
                    data['order_status']) + ',' + str(
                    data['order_submit_status']))
                bdQuery_result['报单检查状态'] = 'end'
    ...
    # --如果是已撤状态的，执行如下
        ...
        if data['order_submit_status'] == Api.const.XTP_ORDER_SUBMIT_STATUS_TYPE[
            ...
            if case_goal['期望状态'] in ('已撤', '内部撤单'):
                # 检查资金持仓是否正确
                fundstkcheck_rs = fundStkCheck_other(QueryInit, QueryEnd, component_stk_info)
                # 如果资金持仓检查正确，则执行报单数据检查，否则结束
                if fundstkcheck_rs['检查状态'] == 'end' and fundstkcheck_rs['测试结果'] == True:
                    # 报单数据检查
...
#--报单为撤单类型的业务处理
#-----------------------------------------------------------------------------------------------------------------------
def etf_cd_type_Handle(Api,wt_reqs,case_goal,data,error,QueryInit,QueryEnd,component_stk_info):
    # 定义测试结果参数
    cdQuery_result = {
        '报单检查状态': 'init',
        '测试结果': False,
...
#--报单查询--未成交-校验
#-----------------------------------------------------------------------------------------------------------------------
def no_match(wt_reqs,case_goal,data,error,trade_fund):
    ...
    if data['qty_traded'] != wt_reqs['quantity']:
        logger.error('未成交状态的报单查询的成交数量应该为'
                     + str(wt_reqs['quantity'])
                     + '，实际成交数量为'+str(data['qty_traded']))
        bdquery_result['报单检查状态'] = 'end'
        bdquery_result['测试结果'] = False
        bdquery_result['错误信息'] = '未成交状态的报单查询的成交数量应该为０'
    # 判断成交金额是否为０
    elif data['trade_amount'] != trade_fund:
        logger.error('未成交状态的报单成交金额应该为'
                     + str(trade_fund)
                     + '，实际成交数量为'+str(data['trade_amount']))
        bdquery_result['报单检查状态'] = 'end'
        bdquery_result['测试结果'] = False
        bdquery_result['错误信息'] = '未成交状态的报单查询的成交金额应该为０'
    # 判断剩余数量是否等于委托数量
    elif data['qty_left'] != 0:
        logger.error('未成交状态的报单剩余数量和委托数量不相等，委托数量和报单剩余数量分别为0',+str(data['qty_left']))
        bdquery_result['报单检查状态'] = 'end'
        bdquery_result['测试结果'] = False
...
def all_match(wt_reqs,case_goal,data,error):
    ...
    # 判断成交金额是否大于零
        ...
        logger.error('全部成交的报单查询成交金额应该不小于０，实际成交数量为'+str(data['trade_amount']))
        bdquery_result['报单检查状态'] = 'end'
        bdquery_result['测试结果'] = False
