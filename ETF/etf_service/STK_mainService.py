...
def ParmIni(Api,expectStatus,price_type):
    ...
    #     result['撤单测试结果'] = True
    #     result['撤单错误原因'] = ''
    if getCancelType(Api,price_type,expectStatus)==False:
        result['撤单检查状态'] = 'end'
        result['撤单测试结果'] = True
        result['撤单错误原因'] = ''
...
def stk_serviceTest(Api,case_goal,wt_reqs):
    # ------------------------------------------------------------------------------------------------------------------
    ...
    # # ------------------------------------------------------------------------------------------------------------------
    # # 报单查询，如果测试用例期望的状态是（中间状态）已报待撤、部撤已报、部成则执行报单查询（异步）------------------------------------
    # # ------------------------------------------------------------------------------------------------------------------
    # if case_goal['期望状态'] in('已报待撤','部撤已报','部成'):
    #     sleep(1)
    #     global request_id,QueryOrder_requestID
    #     QueryOrder_requestID=request_id
    #     # 根据xtpID查询报单
    #     rs_QueryOrder=Api.trade.QueryOrderByXTPID(case_goal['xtp_ID'],QueryOrder_requestID)
    #     request_id += 1
    #
    ...
    while getTimeInt()<=insertOrderTime+t:
        if result['用例检查状态']== 'end':
            ...
            if result['报单检查状态'] == 'end' and result['撤单检查状态'] == 'end' and endCheckFlag == False:
                ...
                elif getCancelType(Api, wt_reqs['price_type'],case_goal['期望状态']) == False and case_goal['是否是撤废'] == '否':
