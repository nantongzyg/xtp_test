...
def etf_caseEndCheck(Api,bdhb_rs, bdquery_rs, cdquery_rs, cjhb_rs, expectStatus,pricetype):
...
def checkDetail_1(bdquery_rs,bdhb_rs,cjhb_rs,expectStatus):
    ...
    #检查xtpID是否一致
        ...
        if expectStatus == '全成':
            result['测试结果'] = True
    # !!etf报单推送，报单查询，成交回报之间暂时不作校验，因etf成交回报推送无顺序，不好判断。
    # else:
    #     if expectStatus == '全成':
    ...
    #         if bdquery_rs['成交数量'] != cjhb_rs['成交数量']:
    #             logger.error(
