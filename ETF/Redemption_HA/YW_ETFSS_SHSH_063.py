...
class YW_ETFSS_SHSH_063(xtp_test_case):
    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_ETFSS_SHSH_063')
        clear_data_and_restart_sh()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_ETFSS_SHSH_063(self):
        # -----------ETF赎回-------------
        title = '上海ETF申购--数量溢出（1000亿）实际最大值'
        # 定义当前测试用例的期待值
