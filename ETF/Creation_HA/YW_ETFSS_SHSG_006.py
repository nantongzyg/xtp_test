...
class YW_ETFSS_SHSG_006(xtp_test_case):

    def test_YW_ETFSS_SHSG_006(self):
        # -----------ETF申购-------------
        ...
        for stk_info in etf_component_info:
            stk_code = stk_info[0]
            components_SZare = QueryEtfComponentsDB(stkparm['证券代码'], stk_code)
            ...
            wt_reqs = {
                ...
                'price':
                    limitup_px,
                'quantity':
                    int(components_SZare * unit_info['component_unit_sell']),
				'position_effect': 
				    Api.const.XTP_POSITION_EFFECT_TYPE['XTP_POSITION_EFFECT_INIT']
            }
            ParmIni(Api, case_goal['期望状态'], wt_reqs['price_type'])
