...
from mfQueryStkassetDecimalDB import *
from mfQueryStkpriceDecimalDB import *
...
#-----入参5：1：取涨停价第三位小数>=5的证券，2：取跌停价第三位小数>=5的证券
# --- 返回值：dict类型 stkparm['证券代码'],stkparm['昨收盘价'],stkparm['涨停价'],stkparm['跌停价'],stkparm['跌停价'],stkparm['随机中间价'],

def QueryStkPriceDecimalQty(stkcode,market,security_type,security_status, trade_status,BSflag,decimal_type,expect_status,Api):
    ...
    if BSflag == 'B':
        rs=QueryStkpriceDecimalDB(stkcode,market,security_type,security_status, trade_status,decimal_type)
        ...
        if stkcode =='999999':
            try:
                ...
                stkcode_list = QueryStkassetDecimalDB(market, security_type,security_status, trade_status,fundacc,decimal_type)
                # 查询符合要求的持仓
                i = 0
        ...
        # 查询价格
        if stkPriceQty['证券代码'] !='':
            stkprice = QueryStkpriceDecimalDB(stkPriceQty['证券代码'], market, security_type, security_status, trade_status,decimal_type)
