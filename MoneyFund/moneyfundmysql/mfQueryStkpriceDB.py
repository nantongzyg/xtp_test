...
def QueryStkpriceDB(stkcode,market,security_type,security_status, trade_status):
    ...
    #
        ...
        if stkcode != '999999':
            str = '''
                select * 
                from xtp_exch_sec_%s
                where exch_id = %s
                and security_type = %s
                and instrument_id = %s
            ''' % (date, market, security_type, stkcode)
        else:
            str = '''
                select * 
                ...
                where exch_id = %s
                and security_type = %s
                and security_status= %s
                and trade_status = %s
                and not exists (select 1 from exch_no_rand_auto where a.instrument_id = stkcode)
            ''' % (date, market, security_type, security_status, trade_status)
    ...
    try:
        ...
        elif security_type_res in(14, 15, 24, 111):
            j=3
