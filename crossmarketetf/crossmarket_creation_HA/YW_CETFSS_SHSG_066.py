...
class YW_CETFSS_SHSG_066(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_CETFSS_SHSG_066')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_CETFSS_SHSG_066(self):
        # -----------ETF申购-------------
        title = '上海ETF申购--深市必须全部现金替代：T-1日无成分股&可用资金恰好→T日申购ETF'
        # 定义当前测试用例的期待值
