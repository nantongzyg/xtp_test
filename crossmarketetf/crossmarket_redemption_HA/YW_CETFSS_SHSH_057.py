...
class YW_CETFSS_SHSH_057(xtp_test_case):

    def setUp(self):
        sql_transfer = SqlData_Transfer()
        sql_transfer.transfer_fund_asset('YW_CETFSS_SHSH_057')
        sql_transfer.update_cur_fee_rate(1, 'YW_CETFSS_SHSH_057')
        clear_data_and_restart_all()
        Api.trade.Logout()
        Api.trade.Login()

    def test_YW_CETFSS_SHSH_057(self):
        # -----------ETF赎回-------------
        title = ('上海ETF赎回--全部成交（数量：最小单位&费用<min）')
        # 定义当前测试用例的期待值
