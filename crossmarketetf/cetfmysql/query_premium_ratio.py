...
def query_premium_ratio(etf_code1):
    ...
    sql = ('SELECT underlying_instrument_id, creation_premium_ratio / 100000, ' +
...
def query_discount_ratio(etf_code1):
    """
    查询etf可现金替代成分股及其溢价比例
    :param etf_code1: etf一级市场代码
    :return:((code1,decimal(0.100),成分股数),(code2,decimal(0.100),成分股数)...)
    """
    date = time.strftime('%Y%m%d', time.localtime(time.time()))
    sql = ('SELECT underlying_instrument_id, redemption_discount_ratio / 100000, ' +
              'component_share from xtp_etf_components_' + date +
              ' where substitute_flag = 1' +
              ' and etf_code1 = ' + str(etf_code1))
    conn = connectMysql()
...
if __name__ == '__main__':
    print query_discount_ratio('510301')
