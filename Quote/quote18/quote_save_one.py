...
def hq_index(data, save_file):
 ...
 # 行情10档买卖盘和买一卖一委托队列字段 
def snap_level_spot(data, save_file):
    stock_code = data['ticker']
    ...
    if data['exchange_id'] == Api.const.XTP_EXCHANGE_TYPE['XTP_EXCHANGE_SH']:
        if str(data['ticker'])[0:3] != '000':
            stock_code = 'SH' + stock_code
            save_snap_level_spot(save_file, stock_code, data, bid1_qty_rs, ask1_qty_rs)
    else:
        if str(data['ticker'])[0:3] != '399':
            stock_code = 'SZ' + stock_code
            save_snap_level_spot(save_file, stock_code, data, bid1_qty_rs, ask1_qty_rs)

def save_snap_level_spot(save_file, stock_code, data, bid1_qty_rs, ask1_qty_rs):
    save_file.write('%s%s%d%s%f'
                '%s%d%s%f%s'
                  '%d%s%d%s%d'
                  '%s%f%s%d%s'
                  '%f%s%d%s'
...
#行情基本字段
def hq_snap_spot(data, save_file):
    stock_code = data['ticker']

    if data['exchange_id'] == Api.const.XTP_EXCHANGE_TYPE['XTP_EXCHANGE_SH']:
        if str(data['ticker'])[0:3] != '000':
            stock_code='SH' + stock_code
            save_hq_snap_spot(save_file, stock_code, data)
    else:
        if str(data['ticker'])[0:3] != '399':
            stock_code = 'SZ' + stock_code
            save_hq_snap_spot(save_file, stock_code, data)

def save_hq_snap_spot(save_file, stock_code, data):
    save_file.write(
                '%s%s%d%s%f%s'
                '%f%s%f%s'
                '%f%s%f%s'
...
#判断股票类型以及市场
def save_md_to_files(data, stock_code, save_file):
        save_file.write(
                '%s%s%d%s%f%s'
                '%f%s%f%s'
                '%f%s%f%s'
...
#订单薄
def save_ob(data, save_file):
    stock_code = data['ticker']

    if data['exchange_id'] == Api.const.XTP_EXCHANGE_TYPE['XTP_EXCHANGE_SH']:
        if str(data['ticker'])[0:3] != '000':
            stock_code = 'SH' + stock_code
            save_ob_stock(save_file, stock_code, data)
    else:
        if str(data['ticker'])[0:3] != '399':
            stock_code = 'SZ' + stock_code
            save_ob_stock(save_file, stock_code, data)

def save_ob_stock(save_file, stock_code, data):
    save_file.write(
                '%s%s%d%s%f%s%d%s'
                '%f%s%d%s'
                '%f%s%d%s'
...
#逐笔委托
def save_tick_by_tick(data, order_spot_file, trade_spot_file):
    stock_code = data['ticker']
