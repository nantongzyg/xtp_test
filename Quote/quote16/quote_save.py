...
def snap_level_spot(data, client_id):
    ...
    bid1_qty_rs = '|'.join(data['bid1_qty_list'])
    ask1_qty_rs = '|'.join(data['ask1_qty_list'])

    if data['exchange_id'] == Api.const.XTP_EXCHANGE_TYPE['XTP_EXCHANGE_SH']:
        if str(data['ticker'])[0:3] != '000':
            stock_code = 'SH' + stock_code
            snap_level_save(client_id, save_level_path, stock_code)
    else:
...
def snap_level_save(client_id, save_level_path, stock_code):
...
def hq_snap_spot(data, client_id):
    ...
    if data['exchange_id'] == Api.const.XTP_EXCHANGE_TYPE['XTP_EXCHANGE_SH']:
        if str(data['ticker'])[0:3] != '000':
            stock_code='SH' + stock_code
            hq_snap_save(client_id, save_snap_path, stock_code)
    else:
        if str(data['ticker'])[0:3] != '399':
            stock_code = 'SZ' + stock_code
            hq_snap_save(client_id, save_snap_path, stock_code)

def hq_snap_save(client_id, save_snap_path, stock_code):
